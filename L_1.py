...
class L1:
    ...
    def getMaxMinDistList(S):
        SS = [L1.L_1_metric(S[0], S[1]), L1.L_1_metric(S[0], S[2]), L1.L_1_metric(S[1], S[2])]
        SS.sort(reverse=True)
        return SS

    @staticmethod
    def getMaxMinDist(S):
        return min(L1.L_1_metric(S[0], S[1]), L1.L_1_metric(S[0], S[2]), L1.L_1_metric(S[1], S[2]))
    ...
    def get_S(self, P, smallest):
        ...
        key_dim = lambda x:  L1.L_1_metric(pa, x)  # Kljuc za primerjanje v dimenziji (dim)
        ...
        while len(P) > 1:
            ...
            dpi = L1.L_1_metric(pa, pi)  # d(pa,pi)
            ...
            U = L1.L_1_metric(pbU, pcU)
            D = L1.L_1_metric(pbD, pcD)
        ...
        if wentRight and hright is not None:
            ...
            return max([(pa, pl, ph), (pa, pl, hright), (pa, lright, ph), (pa, lright, hright)], key=L1.getMaxMinDist)
        else:
            ...
            return max([(pa, pl, ph), (pa, pl, P[0]), (pa, P[0], ph)], key=L1.getMaxMinDist)
